...
public class LocalNotification extends CordovaPlugin {
      ...
      .execute(
        new Runnable() {
          public void run() {
            ...
            } else if (action.equals("notificationCallback")) {
              notificationCallback(args, command);
            }
          }
        }
      );
  ...
  /** */
  private void notificationCallback(JSONArray args, CallbackContext command) {
  ...
  private static boolean isInForeground() {
    if (!deviceready || webView == null) return false;

    CordovaWebView view = webView.get();

    KeyguardManager km = (KeyguardManager) view
      .getContext()
